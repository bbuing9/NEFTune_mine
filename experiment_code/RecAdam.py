...
class RecAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-6, weight_decay=0.0, correct_bias=True,
                 anneal_fun='linear', anneal_k=0, anneal_t0=0, anneal_w=1.0, pretrain_cof=5000.0, pretrain_params=None):
        if lr < 0.0:
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if group['anneal_w'] > 0.0:
                    ...
                    p.data.add_(p.data - self.orig_weights[p], alpha=-group["lr"] * (group['anneal_w'] - anneal_lambda) * group["pretrain_cof"])
                else:
