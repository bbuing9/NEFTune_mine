...
def calculate_fisher(args, model, dataloader):
    ...
    for name, params in model.named_parameters():
        if 'layer' in name:
            ...
            gradient_mask2[params] = params.new_zeros(params.size())
            
    N = len(dataloader)
    epoch_iterator = iter(dataloader)
    # Calculation Fisher Information for Instruction tuning 
    ...
    # Calculation Fisher Information for Preservation (KD-loss)
    epoch_iterator = iter(dataloader)
    ...
    for step_count in range(int(0.1 * args.max_steps)):
        ...
        for _ in range(args.accumulation_steps):
            ...
            (kd_loss/args.accumulation_steps).backward()

        for name, params in model.named_parameters():
            if 'layer' in name:
                torch.nn.utils.clip_grad_norm_(params, args.max_grad_norm)
                gradient_mask2[params] += (params.grad ** 2) / N
        model.zero_grad()
    ...
    for k, v in gradient_mask.items():
        ...
        v2 = gradient_mask2[k].view(-1).cpu().float().numpy()
        if r2 is None:
            r2 = v2
        else:
            r2 = np.append(r2, v2)
    polar = np.percentile(r, (1-args.reverse_p)*100)
    polar2 = np.percentile(r2, (1-args.reverse_p)*100)
    if args.select == 'cap':
        for k, v in gradient_mask.items():
            gradient_mask[k] = (gradient_mask[k] > polar) * (gradient_mask2[k] > polar2)
    elif args.select == 'sub':
        for k, v in gradient_mask.items():
            gradient_mask[k] = (gradient_mask[k] > polar) * (gradient_mask2[k] < polar2)
    else:
        raise ValueError("Incorrect selection criteria (available: cap | sub)")
        ...
        for _ in range(accumulation_steps):
            ...
            if args.self_kd > 0:
                ...
                if args.mask_p > 0:
                    ...
                    selected_probs = shift_logits.view(-1, vocab_size).softmax(dim=-1)[torch.arange(len(shift_labels)), shift_labels]
